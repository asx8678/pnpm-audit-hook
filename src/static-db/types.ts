...
/**
 * The runtime interface for the static vulnerability database.
 */
export interface StaticDatabase {
  /** Get the database index (metadata and package list) */
  getIndex(): Promise<StaticDbIndex>;

  /** Check if a package has any known vulnerabilities (O(1) via index) */
  hasVulnerabilities(packageName: string): Promise<boolean>;

  /** Get all vulnerabilities for a package with optional filtering */
  query(packageName: string, options?: StaticDbQueryOptions): Promise<StaticDbQueryResult>;

  /** Get vulnerabilities for multiple packages in parallel */
  queryBatch(
    packages: Array<{ name: string; version: string }>,
    options?: Omit<StaticDbQueryOptions, "version">
  ): Promise<Map<string, StaticDbQueryResult>>;
}
...
/**
 * Get numeric severity level for comparison (higher = more severe).
 */
...
export function compareSeverity(a: Severity, b: Severity): number {
  return severityLevel(a) - severityLevel(b);
}
